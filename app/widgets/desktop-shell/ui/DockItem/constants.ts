/*
 * Dock icons — one entry per `icon` name used in `app.config.ts` `dock`.
 * Real brand marks (GitHub, LinkedIn, Telegram — Simple Icons, CC0) plus a
 * download glyph for the résumé. Each value is a list of SVG path `d` strings
 * on a 24×24 grid, filled with `currentColor` (`fill-rule: evenodd`).
 */
