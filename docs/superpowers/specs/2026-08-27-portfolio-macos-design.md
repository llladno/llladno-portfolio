...
## 4. Intro: код-хореография («фото → въезд в экран → рабочий стол»)

> **Пересмотрено 2026-08-29.** Кадровый скраб из mp4 (canvas image sequence,
> `ScrubCanvas`, `pnpm intro:frames`) заменён на хореографию по коду поверх
> реального DOM — из-за того, что ИИ-видео перехода стабильно выглядело
> устаревшим, а кадровый пайплайн нёс лишний вес и препроцессинг. Прототип:
> валидирован вживую до внедрения. Второй проход: «въезд в экран» + `ScreenGlow`
> заменены на **параллакс фото** — первый экран должен читаться как обычный
> лендинг, macOS-элементы просто появляются при скролле.

### Структура сцены

- `app/pages/index.vue` → `<IntroStage>` + далее секции.
- `IntroStage` — контейнер высотой `INTRO_STAGE_HEIGHT_VH` (≈`340vh`); внутри
  `position: sticky` вьюпорт-слой (CSS, без JS-пина). Слушатель `scroll`
  (throttle через `requestAnimationFrame`, `getBoundingClientRect`) считает
  общий прогресс `0→1`; `gsap.set` применяет состояние каждого слоя.
- Слой 1 — **лендинг**: `HeroLayer` (портрет `public/hero.jpg` + запасная
  CSS-сцена «студия») во весь экран за градиентным `intro-scrim`, сверху H1 +
  подзаголовок. Никакого macOS-хрома. Слои выше: `wallpaper-surface`
  (кроссфейд-цель), `IntroCopy`.
- Фазы прогресса:
  1. `IntroCopy` затухает и уезжает вверх (`INTRO_COPY_FADE_SPEED`).
  2. Фото **параллаксит** — дрейф вверх (`PARALLAX_SHIFT_PERCENT`) + лёгкий
     скейл (`PARALLAX_SCALE_*`), это НЕ зум в экран. Скрим углубляется
     (`SCRIM_OPACITY_*`).
  3. `TL_WALL_START…END` — `wallpaper-surface` проявляется поверх фото.
  4. На `BOOT_PROGRESS_THRESHOLD` → `boot()`; `DesktopShell` со stagger'ом
     выводит menu bar (сверху), Dock (снизу), первое окно.

### Техника

- Только реальный DOM + CSS-трансформы: ни генеренного видео, ни канваса, ни
  ffmpeg. Тайминги/кривые — константы, полностью управляемы.
- GSAP (`gsap.set`, интерполяция `filter`) грузится динамическим `import()`
  в `onMounted` — вне SSR-пути и критического рендера. До загрузки держатся
  CSS-начальные состояния (`opacity: 0` у glow/wall) — без вспышки.
- Ветка кинематографик гейтится флагом `isMounted`, чтобы первый клиентский
  кадр совпадал с SSR (иначе hydration mismatch на `isDesktop`).
- `--color-wall-*` / `--color-glow-*` уведены в тёплую чёрно-оранжевую гамму
  под фото; `@utility wallpaper-surface` в `main.css` общий для `<Wallpaper>` и
  intro-слоя, поэтому стык свечение → обои бесшовный.

### Fallback (мобилка / `prefers-reduced-motion` / повторный визит / no-JS)

- Ни скраба, ни sticky. `IntroFallback`: статичный герой на фоне `hero.jpg`
  (+ `wallpaper-surface` как запас) + H1 + подзаголовок + кнопка «Смотреть
  работы» → скролл к секциям. Немедленный `boot()`.
- Рабочий стол на мобилке не собирается — сразу `plain` layout.
- SSR/SSG всегда отдаёт этот семантический вариант (он несёт единственный H1);
  кинематографик — улучшение поверх.

### Ассеты

- `public/hero.jpg` — портрет 16:9, первый кадр интро (оптимизировать:
  WebP/AVIF, ~1600px). Больше никаких intro-ассетов.
